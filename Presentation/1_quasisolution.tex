...
\note{
    \begin{itemize}
        ...
        у объекта на части границы неизвестны отражательная способность на части границы...
        \item Об обратной задаче нет данных - формулируем задачу опт.\ управления.
        \item Квазирешение.\ Доказано существование, разрешимость.
    \end{itemize}
}
...
\note{
    \begin{itemize}
        ...
        \item Основанный на представленной системе оптимальности, предложенный и реализованный алгоритм представлен на слайде.
        ...
        \item Выбор градиентного метода обусловлен дифузионной природой модели, возможностью контроля, простотой, возможностью модификации
        \item Отметим, что в силу невыпуклости экстремальной задачи градиентные алгоритмы
        \item Однако свойства диффузионных моделей сложного теплообмена
    \end{itemize}
...
%        \caption*{$\Gamma \coloneqq \partial \Omega =\overline{\Gamma}_0 \cup \overline{\Gamma}_1 \cup \overline{\Gamma}_2$}
    ...
    $u(x)=0.49x+0.01$.
    \\
    Из решения краевой задачи получим $\theta_b$,
...
\begin{itemize}
    ...
    \item Для заранее известной функции управления рассчитаем состояние, и температуру с доступной границы положив в ф.\ качества.
    \end{itemize}
}
...
\note{
    \begin{itemize}
        \item На верхних графиках представлен результаты работы алгоритма.
        \item На нижних -- динамика функционала качества.
        ...
        \item Алгоритм существенно замедляется и далее требуется большое число итераций
...
\note{
    \begin{itemize}
        \item Первая из рассмотренных задач с условиями Коши.\ Известна только температура и тепловой поток.
        \item Однозначная разрешимость -- 2019 год (АЮ Чеботарёв),
        \item Однако способ нахождения решения, а также его теоретическое обоснование -- в диссертации.
        \item Для этого используем замену~\eqref{eq:2_2:bc3}. $r$ - известна, $u$ - управление.
...
\note{
    \begin{itemize}
        \item Доказано существование решения оптзадачи, выведена система оптимальности.
        \item Численный алгоритм, основанный на системе оптимальности~\eqref{eq:2_2:as} представлен на слайде.
        \item Приведём один из экспериментов из диссертации.
    \end{itemize}
...
\note{
    \begin{itemize}
        \item Используя параметры среды, а также значения температуры и теплового потока как
        указано на слайде, применим предложенный алгоритм.
        \item Полученное решение для температуры соответствует заданному, хотя $\theta_b$
        фигурирует только в функционале качества.
        \item Начальное и конечное значение функционала качества (в этом и других численных экспериментах)
        демонстрирует высокую эффективность предложенного алгоритма.
    \end{itemize}
}
