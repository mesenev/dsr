...
4.\ Наконец, начальное условие представляется четвертым уравнением
...
Здесь $\Sigma = \Gamma \times (0,T)$, через $U$ обозначено пространство $L^{2}(\Sigma)$ с нормой
\[
...
\begin{proof}
    ...
    Пусть
    \[
        [\zeta, \eta]=\left(\left(\alpha I+\kappa_{a} A^{-1}\right) \zeta,
    \eta\right), \zeta \in V^{\prime}, \eta \in V.
    \]
    Обратим внимание, что выражение $[[\eta]]=\sqrt{[\eta, \eta]}$
