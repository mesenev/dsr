...
(i) $\alpha, \beta, \sigma \in L^{\infty}(\Omega),
\quad b=r \beta, r=$ Const $>0 ; \alpha \geq \alpha_{0}, \beta \geq \beta_{0},
\sigma \geq \sigma_{0}, \alpha_{0}, \beta_{0}, \sigma_{0}=$ Const $>0$.

(ii) $0<k_{0} \leq k(s) \leq k_{1},\left|k^{\prime}(s)\right| \leq k_{2},
s \in \mathbb{R}, \quad k_{j}=$ Const.

(iii) $0 \leq \theta_{b} \in L^{\infty}(\Sigma), 0 \leq \theta_{\text{в}}
...
\begin{lemma}
    \label{lm:1_6:1}
    Пусть выполняются условия (i) - (iv), $0 \leq \theta \leq w(t), \varphi=F_{1}(\theta)$.
    В таком случае
    \begin{equation}
        \label{eq:1_6:8}
...
\begin{lemma}
    \label{lm:1_6:2}
    Пусть выполняются условия (i) - (iv),
...
\begin{lemma}
    \label{lm:1_6:3}
    Если выполнены условия (i)-(iv), то существует константа $C>0$,
...
\begin{theorem}
    \label{th:1_6:1}
    Если выполнены условия (i)-(iv), то существует хотя-бы одно
...
\begin{theorem}
    \label{th:1_6:2}
    Если выполнены условия (i) - (iv) и $\theta_{*}, \varphi_{*}$ являются
